    ...
    >
        @foreach ($data as $resource)
            <x-filament::grid.column>
                <x-filament::section
                    class="overlook-card rounded-xl overflow-hidden relative h-16 bg-gradient-to-tr from-gray-100 via-white to-white dark:from-gray-950 dark:to-gray-900"
                >
                    <a
                        href="{{ $resource['url'] }}"
                    ...
                    >
                        @if ($resource['icon'])
                            <x-filament::icon
                                :icon="$resource['icon']"
                                :size="10"
                                class="overlook-icon w-auto h-10 absolute left-0 top-8 text-primary-500 opacity-20 dark:opacity-20 transition group-hover:scale-110 group-hover:-rotate-12 group-hover:opacity-40 dark:group-hover:opacity-80"
                            />
                        @endif
                    </a>
                    <div style="left: 15%;position: absolute;">
                        <span class="overlook-name mx-3">{{ $resource['name'] }}</span>
